...
ExplosionEffect::ExplosionEffect(const sf::Vector2f position, const int direction) : SpriteEffect(explosion_effect, sprite_params.animations[0].get_total_time(), position), direction_(direction)
{
	sprite_.setScale(sprite_params.scale);
}
...
sf::Vector2f ExplosionEffect::get_offset() const
{
	...
	return { offset.x * static_cast<float>(direction_) - static_cast<float>(2 * sprite_params.frame_width), offset.y };
}
